...
def update_match_info():
    """Returns list of new integer match_ids added."""
    ...
    print(f"[match_info] existing rows: {len(existing_filenames)}")
    ...
    with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
        ...
        print(f"[match_info] total YAML files: {len(yaml_files)}")
    ...
    write_csv(full_df, MATCH_INFO_CSV_KEY)   # write CSV to separate prefix so Athena doesn't scan it
...
def handler(event, context):
    ...
    new_match_ids = update_match_info()
    if new_match_ids:
        update_ball_by_ball(new_match_ids)
        update_batter_scorecard(new_match_ids)
    else:
        print("[ingestion] no new matches — nothing else to do")
    return {
        "statusCode": 200,
        "body": f"Done. New matches added: {len(new_match_ids)}",
    }
